...
(
	...
	inner join 
	(
		select  Ev.matr,sum(V.valor) as valor from emp_venc Ev
		inner join vencimento V on Ev.cod_venc=V.cod_venc
		group by Ev.matr
		order by Ev.matr
	) as B on B.matr=E.matr
) as BRUTO
inner join 
---Trazendo o desconto
(
	select C.matr, E.Nome,C.valor as Descontos  from empregado E 
	inner join 
	(
		select  Ed.matr,sum(D.valor) as valor from emp_desc Ed
		inner join desconto as D on Ed.cod_desc=D.cod_desc
		group by Ed.matr
		order by Ed.matr
